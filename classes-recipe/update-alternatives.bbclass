...
def update_alternatives_enabled(d):
    ...
    # Disable based on the 'noalternatives' DISTRO_FEATURES
    # this might for example be set when ostree which is
    # incompatible with alternatives is in use.
    if bb.utils.contains('DISTRO_FEATURES', 'noalternatives', True, False, d):
        return False
